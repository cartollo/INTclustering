...
pdf(file = paste0(folder,"/","comparefolderouput_create_dismatrix_correlation_average_NOWHITE_CZM_iscore_shotgun.pdf")) 
...
for (bcase in files) {
  ...
  if(max(colSums(res$contingency_table))>=90)
    cat("trovato cluster merdoso",max(colSums(res$contingency_table))," ", bcase,"\n")
